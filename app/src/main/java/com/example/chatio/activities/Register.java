...
public class Register extends AppCompatActivity {
    ...
    private String encodedImage; // to put image-> undecided
...
*/
    ...
    //Somehow doesn't work to go back-> this method deprecated
   /* private void setListeners() {
      ...
      /*  private String encode Image (Bitmap bitmap) {
        int previewWidth = 150;
        int previewHeight = bitmap.getHeight() * previewWidth / bitmap.getWidth();
        Bitmap preview Bitmap = Bitmap.createScaled Bitmap(bitmap, previewWidth, previewHeight, filter: false);
        ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
        previewBitmap.compress (Bitmap.Compress Format.JPEG, quality: 50, byteArrayOutputStream);
        byte[] bytes byteArrayOutputStream.toByteArray();
        return Base64.encodeToString(bytes, Base64.DEFAULT);
    }
*/

    /*    private final Activity Result Launcher<Intent> pickImage = registerForActivityResult(
            new ActivityResultContracts. StartActivity ForResult(),
            result -> {
